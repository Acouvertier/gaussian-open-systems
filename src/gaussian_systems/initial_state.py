...
from ._validation import _valid_mode_number, _valid_nbars_array, _valid_mean_covariance_tuple, _valid_indices, _valid_parameter_tuple, _valid_mean_covariance, _require_real_scalar, _require_nonnegative_real_scalar, _require_positive_integral_scalar
...
def thermal_vacuum_covariance(n: Integral, nbars:npt.NDArray[np.float64]|None=None) -> npt.NDArray[np.float64]:
    ...
    if nbars is None:
        ...
        if n > len(nbars):
            ...
            warnings.warn(f"Warning: {n}-mode system but only {len(nbars)} thermal occupations provided. Assuming 0 thermal occupation for modes {len(nbars)+1}-{n}.")
